...
function DisapperingImage(props) {
	...
	useEffect(() => {
		...
		imgRef.current.dataset.watermark = (props.state.myInfo.ipInfo.ip + "  -|-  ").repeat(props.state.isMobileView ? 5 : 10);
	}, []);

	useEffect(() => {
		if (props.msg.shouldDisappear) {
			if (time === 0) {
				if (imgRef.current) imgRef.current.style.filter = "blur(7px)";
			} else {
				setTimeout(() => {
					setTime(time - 1);
				}, 1000);
